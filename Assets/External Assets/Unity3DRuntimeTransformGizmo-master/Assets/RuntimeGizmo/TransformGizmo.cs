...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		void Awake()
		{
			...
			GameManager.Instance.OnTargetObjectSelected += (object sender, Target target) => GetTarget(target.transform);
			GameManager.Instance.OnTargetObjectRemoved += (object sender, Target target) => ClearTargets();
		}
		...
		public void SetTransformTypeToMove()
        {
			transformType = TransformType.Move;
		}
		public void SetTransformTypeToRotate()
		{
			transformType = TransformType.Rotate;
		}
		public void SetTransformTypeToScale()
		{
			transformType = TransformType.Scale;
		}

		public bool IsTheSameTransformType(TransformType type)
        {
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					if(transType == TransformType.Move)
					{
						...
						for(int i = 0; i < targetRootsOrdered.Count; i++)
						{
							...
							PropertyPanel.Instance.UpdateTransformPanel(target.transform);
						}
		...
		void GetTarget()
		{
			if(nearAxis == Axis.None && Input.GetMouseButtonDown(0))
			{
				...
				{
					if (hitInfo.collider == null || hitInfo.collider.GetComponent<ISelectable>() == null)
                    {
						return;
                    }
